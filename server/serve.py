...
ESP32_IP = os.environ.get("ESP32_IP", "")  # Set to ESP32's IP to send translated text via UDP (local/LAN mode)

# Latest translated subtitle so ESP32 devices can poll over HTTP from *any* network.
# This avoids relying on UDP to a private LAN IP.
LATEST_TEXT = ""
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        """
        Simple polling endpoint for ESP32:
        - GET /latest-subtitle  -> returns latest translated subtitle as text/plain (UTF-8)
        This works even when ESP32 and phone are on different networks,
        as long as they can both reach this server over the internet.
        """
        global LATEST_TEXT

        if self.path == "/latest-subtitle":
            self.send_response(200)
        ...
        # Fall back to normal file serving behavior
        super().do_GET()
    ...
    def do_POST(self):
        if self.path == "/translate":
            ...
            try:
                ...
                # Store latest translated subtitle for HTTP-polling ESP32 clients
                global LATEST_TEXT
                LATEST_TEXT = result or ""

                # Optional: still send via UDP when ESP32_IP is reachable on same LAN
